...
function move(id, position, color) {
    ...
    if(id == 1){
      ...
      texto.textContent = 'você está na home do app'
    }
    if(id == 2){
      titulo.textContent ='REDE SOCIAL';
      texto.textContent = 'sem atualizações'
    }
    if(id == 3){
      titulo.textContent ='ALERTAS';
      texto.textContent = 'sem notificações'
    }
    if(id == 4){
      titulo.textContent ='USER';
      texto.textContent = 'edite o seu usuário'
    }
